...
export const generateAnalysis = async (
  ...
  try {
    ...
    // Function to validate and fix truncated JSON
    function fixTruncatedJson(str: string): string {
      ...
      // If we have unclosed arrays or objects, try to close them properly
    ...
    // Function to ensure currency consistency throughout the response
    function fixCurrencyConsistency(obj: any, currency: string): any {
    ...
    if (!resultObj) {
      ...
      throw new Error('The AI response could not be properly formatted. Please try again with a more specific business idea.');
    }

    // Currency consistency check and fix
    if (resultObj.currency) {
